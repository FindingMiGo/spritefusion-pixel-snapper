...
fn process_image_bytes_common(input_bytes: &[u8], config: Option<Config>) -> Result<Vec<u8>> {
    ...
    let quality_report =
        build_quality_report(&rgba_img, &quantized_img, &output_img, &col_cuts, &row_cuts);
    println!("{}", quality_report.summary_line());

    #[cfg(not(target_arch = "wasm32"))]
    if let Some(report_path) = &config.report_path {
        std::fs::write(report_path, quality_report.to_text()).map_err(|e| {
            PixelSnapperError::ProcessingError(format!("Failed to write quality report: {}", e))
        })?;
    ...
    if let Some(diff_path) = &config.diff_path {
        let diff_img = build_diff_image(&quantized_img, &output_img, &col_cuts, &row_cuts);
        diff_img
            .save(diff_path)
            .map_err(PixelSnapperError::ImageError)?;
        println!("Diff mask saved to: {}", diff_path);
    }
...
fn parse_args() -> Option<Config> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--report" => {
                let Some(val) = args.get(i + 1) else {
                    eprintln!("Warning: --report requires an output path");
                    break;
                };
                config.report_path = Some(val.clone());
                i += 2;
            }
            "--diff" => {
                let Some(val) = args.get(i + 1) else {
                    eprintln!("Warning: --diff requires an output PNG path");
                    break;
                };
                config.diff_path = Some(val.clone());
                i += 2;
            }
...
struct QualityReport {
    ...
    grid_height: usize,
    grid_regularity: f64,
    color_similarity: f64,
    pixels_changed: f64,
    mean_color_error: f64,
...
impl QualityReport {
    fn summary_line(&self) -> String {
        format!(
            "Quality: grid {:.1}%, color {:.1}%, changed {:.1}%, error {:.2}",
            self.grid_regularity, self.color_similarity, self.pixels_changed, self.mean_color_error
        )
    }
...
fn build_quality_report(
    ...
    quantized_img: &RgbaImage,
    output_img: &RgbaImage,
    col_cuts: &[usize],
    row_cuts: &[usize],
) -> QualityReport {
    let (color_similarity, pixels_changed, mean_color_error, opaque_pixels) =
        diff_metrics(quantized_img, output_img, col_cuts, row_cuts);
    QualityReport {
        grid_width: col_cuts.len().saturating_sub(1),
        grid_height: row_cuts.len().saturating_sub(1),
        grid_regularity: grid_regularity(col_cuts, row_cuts),
        color_similarity,
        pixels_changed,
        mean_color_error,
        reconstruction_error: reconstruction_error(source_img, col_cuts, row_cuts),
        opaque_pixels,
    }
}

fn grid_regularity(col_cuts: &[usize], row_cuts: &[usize]) -> f64 {
    let irregularity = (cut_irregularity(col_cuts) + cut_irregularity(row_cuts)) / 2.0;
    ((1.0 - irregularity.min(1.0)) * 100.0).clamp(0.0, 100.0)
}
...
fn diff_metrics(
    ...
    let max_distance = (3.0f64 * 255.0 * 255.0).sqrt();

    for (y_i, row_pair) in row_cuts.windows(2).enumerate() {
        let y0 = row_pair[0].min(quantized_img.height() as usize);
        let y1 = row_pair[1].min(quantized_img.height() as usize);
        if y1 <= y0 || y_i >= output_img.height() as usize {
            continue;
        }
        for (x_i, col_pair) in col_cuts.windows(2).enumerate() {
            let x0 = col_pair[0].min(quantized_img.width() as usize);
            let x1 = col_pair[1].min(quantized_img.width() as usize);
            if x1 <= x0 || x_i >= output_img.width() as usize {
                continue;
            }
            let out = output_img.get_pixel(x_i as u32, y_i as u32).0;
            for y in y0..y1 {
                for x in x0..x1 {
                    let src = quantized_img.get_pixel(x as u32, y as u32).0;
                    if src[3] == 0 && out[3] == 0 {
                        continue;
                    }
                    let distance = color_distance(src, out);
                    total_distance += distance;
                    if distance > 0.5 || src[3].abs_diff(out[3]) > 0 {
                        changed += 1;
                    }
                    opaque_count += 1;
                }
    ...
    let mean_error = total_distance / opaque_count as f64;
    let color_similarity = ((1.0 - mean_error / max_distance) * 100.0).clamp(0.0, 100.0);
    let pixels_changed = changed as f64 * 100.0 / opaque_count as f64;
    (color_similarity, pixels_changed, mean_error, opaque_count)
}
...
fn build_diff_image(
    ...
    let mut diff_img = RgbaImage::new(quantized_img.width(), quantized_img.height());
    for (y_i, row_pair) in row_cuts.windows(2).enumerate() {
        let y0 = row_pair[0].min(quantized_img.height() as usize);
        let y1 = row_pair[1].min(quantized_img.height() as usize);
        if y1 <= y0 || y_i >= output_img.height() as usize {
        ...
        for (x_i, col_pair) in col_cuts.windows(2).enumerate() {
            ...
            for y in y0..y1 {
                for x in x0..x1 {
                    let src = quantized_img.get_pixel(x as u32, y as u32).0;
                    let distance = color_distance(src, out);
                    let alpha_delta = src[3].abs_diff(out[3]) as f64;
                    let intensity = ((distance + alpha_delta) / 2.0).clamp(0.0, 255.0) as u8;
                    let pixel = if intensity == 0 {
                        Rgba([0, 0, 0, 0])
                    } else {
                        Rgba([255, 32, 96, intensity.max(48)])
                    };
                    diff_img.put_pixel(x as u32, y as u32, pixel);
                }
            }
        }
    }
